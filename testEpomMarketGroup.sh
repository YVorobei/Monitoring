...
#----------------*** 1Check Safebrowsing ***--------------------
		...
		if ((count_grep == 0))
		   then
			 ...
			 echo OK  $checking_site $safebrowsing
	   	   else 
			 echo $checking_site safebrowsing -- False >> group_Maxiget.log
			 echo False $checking_site $safebrowsing
		fi
...
#----------------*** 2Check Siteadvisor ***--------------------
		...
		if (($? == 0))
		   then
			 ...
			 echo OK $checking_site  $siteadvisor
		   else 
			 echo $checking_site siteadvisor -- False >> group_Maxiget.log
			 echo False $checking_site $siteadvisor
		fi
			...
			grep 'No active malware was reported recently by users anywhere on this website' sourse_site.log > /dev/null 2>&1
				...
				if (($count_grep == 0))
					then
						...
						echo OK $checking_site $avgthreatlabs
					else 
						echo $checking_site avgthreatlabs -- False >> group_Maxiget.log
						echo False $checking_site  $avgthreatlabs
				fi
...
#----------------*** 4Check Sitecheck_sucuri ***--------------------
		...
		if (($count_grep == 0))
			then
				...
				echo OK $checking_site $sitecheck_sucuri
			else 
				echo $checking_site sitecheck_sucuri -- False >> group_Maxiget.log
				echo False $checking_site $sitecheck_sucuri
		fi
...
#----------------*** 5Check Quttera ***--------------------
		...
		if (($count_grep == 0))
			then
				...
				echo OK $checking_site $quttera
			else 
				echo $checking_site quttera -- False >> group_Maxiget.log
				echo False $checking_site $quttera
		fi
